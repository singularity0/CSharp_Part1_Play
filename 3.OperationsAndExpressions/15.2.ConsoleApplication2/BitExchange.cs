...
//Solution the hard way. solution first extracts all bits. then it compares the values for bit value of bit 3 and bit 24 and depending on the value it exchanges their values or does nothing. Same for the rest of the bits
...
class BitExchange
{
    static void Main(string[] args)
    {
        ...
        if (bit1 != bit4)
        {
            ...
            {
                long mask7 = 1 << 24;
                long nOrMask = numberTrans | mask7;
                long mask8 = ~(1 << 3);
            ...
            {
                long mask9 = ~(1 << 24);
                long nAndMask8 = numberTrans & mask9;
                long mask10 = 1 << 3;
